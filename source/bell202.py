...
async def vos_main():
    ...
    def toggle_xy(key):
        ## not right yet
        global bell202_state
        global d
        if bell202_state["selected_axis"] == 0:
            bell202_state["selected_axis"] = 1
            keyboardio.KeyboardIO.leds |= (1<<6)
            keyboardio.KeyboardIO.leds &= ~(1<<7)
        else:
            bell202_state["selected_axis"] = 0
            keyboardio.KeyboardIO.leds |= (1<<7)
            keyboardio.KeyboardIO.leds &= ~(1<<6)
        
        ## Update leds to reflect switch
        keyboardio.KeyboardIO.leds &= (0b11000011)
        which_led = bell202_state["waves_leds"][bell202_state["selected_axis"]] 
    ...
    def toggle_waveform(key):
        ...
        keyboardio.KeyboardIO.leds |= (1<<(5-which_led ))
        ## update storage
        bell202_state["waves_leds"][bell202_state["selected_axis"]] = which_led
        ## update waveform
        d.waveform[bell202_state["selected_axis"]] = _waves_lookup[which_led]
        d.recalculate_waveforms()
